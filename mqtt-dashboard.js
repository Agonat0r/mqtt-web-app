...
async function sendSmsAlert(type, message) {
    ...
    try {
        // For each phone number, try to send via email gateway
        for (const phone of phones) {
            // Get carrier from phone number's data attribute
            const phoneItem = phoneList.querySelector(`[data-phone="${phone}"]`);
            const carrier = phoneItem ? phoneItem.dataset.carrier : 'att'; // Default to AT&T if not specified
            
            const gateway = carrierGateways[carrier];
            if (!gateway) {
                console.warn(`No gateway found for carrier: ${carrier}`);
                continue;
            }

            const smsEmail = phone.replace(/[^\d]/g, '') + gateway;
            
            // Use EmailJS to send the SMS via email gateway
            await emailjs.send(
            ...
            console.log(`SMS alert sent via email gateway to ${phone}`);
        }
    } catch (error) {
...
// Modify phone number addition to include carrier selection
function addPhoneToList(phone, carrier = 'att') {
    const phoneItem = document.createElement('div');
    phoneItem.className = 'phone-item';
    phoneItem.innerHTML = `
        <span class="phone-text" data-phone="${phone}" data-carrier="${carrier}">${phone}</span>
        <select class="carrier-select" onchange="updateCarrier(this)">
            ${Object.keys(carrierGateways).map(c => 
                `<option value="${c}" ${c === carrier ? 'selected' : ''}>${c.toUpperCase()}</option>`
            ).join('')}
        </select>
...
// Function to update carrier for a phone number
function updateCarrier(select) {
    const phoneItem = select.closest('.phone-item');
    const phoneSpan = phoneItem.querySelector('.phone-text');
    phoneSpan.dataset.carrier = select.value;
    savePhones();
}

// Modify savePhones to include carrier information
function savePhones() {
    const phones = Array.from(phoneList.querySelectorAll('.phone-text'))
        .map(span => ({
            number: span.dataset.phone,
            carrier: span.dataset.carrier
        }));
    localStorage.setItem('smsAlerts', JSON.stringify(phones));
}

// Modify the phone loading code
window.addEventListener('load', () => {
    ...
    savedPhones.forEach(phone => {
        if (typeof phone === 'string') {
            ...
            // Handle new format with carrier
            addPhoneToList(phone.number, phone.carrier);
        }
    });
